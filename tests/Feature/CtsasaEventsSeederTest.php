...
it('seeds CTSASA and the remaining 2026 championships from the entry-forms page', function () {
    ...
    expect($events)->toHaveCount(5)
        ->and($events->first()->title)->toBe('SA National English Sporting Championship')
        ...
        ->and($events->last()->title)->toBe('Northern Cape Standard Championships')
        ->and($events->last()->starts_at->toDateString())->toBe('2026-10-31')
        ->and($events->last()->venue?->name)->toBe('Maccauw Clay Target Club');
